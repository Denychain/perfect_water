...
document.addEventListener('DOMContentLoaded', () => {
        ...
        li.innerHTML = `
            <div class="review-card__header">
                <img 
                    ...
                    alt="${review.name}" 
                    loading="lazy"
                    onerror="this.src='https://ui-avatars.com/api/?name=${safeName}&background=0D1B2A&color=fff&size=64'"
                >
            </div>
            <div class="review-card__name">${review.name}</div>
            
            <div class="review-card__stars" aria-label="Оцінка ${review.rating} з 5">
    ...
    async function loadReviews() {
        try {
            ...
            track.innerHTML = '<p style="text-align:center; padding: 20px;">Не вдалося завантажити відгуки. Спробуйте пізніше.</p>';
        }
    }
...
function initReviewsNavigation() {
    ...
    if (dotsContainer) {
        ...
        viewport.addEventListener('scroll', () => {
            ...
            const isAtEnd = viewport.scrollLeft + viewport.offsetWidth >= viewport.scrollWidth - 5;
            ...
            if (isAtEnd) {
                ...
                cards.forEach((card, index) => {
                    const diff = Math.abs(card.getBoundingClientRect().left - viewport.getBoundingClientRect().left);
                    if (diff < minDiff) {
                        minDiff = diff;
                        bestIndex = index;
                    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function createProductCard(product) {
        ...
        article.innerHTML = `
            <a class="product-card__media" href="${link}" aria-label="${product.title} — детальніше">
                <img
                    class="product-card__img"
                    src="${product.image}"
                    alt="${product.title}"
                    loading="lazy"
                    onerror="this.src='https://placehold.co/300x300/F7FAFC/1C2A33?text=No+Image'" 
                />
            </a>

            <div class="product-card__body">
                <h3 class="product-card__title">
                    <a class="product-card__titleLink" href="${link}">${product.title}</a>
                </h3>
                ...
                <div class="product-card__actions">
                    ...
                    <button class="product-card__btn product-card__btn--secondary" type="button" aria-label="Підібрати ${product.title}">
                        Підібрати цю
                    </button>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const triggers = document.querySelectorAll('#calc-trigger-card, #calc-trigger-btn');
    ...
    const totalQuestions = quizSteps.filter(s => s.type !== 'loading' && s.type !== 'form').length;
    ...
    function renderStep(index) {
        ...
        if (step.type !== 'loading' && step.type !== 'form') {
            ...
            stepInfoEl.innerHTML = `Крок <span id="calc-step-num">${index + 1}</span> з ${totalQuestions}`;
    ...
    function renderQuestionState(step) {
        ...
        step.options.forEach(opt => {
            ...
            label.innerHTML = `
                <input class="calc-option__input" type="${inputType}" name="${step.id}" value="${opt.value}" ${isChecked ? 'checked' : ''}>
        ...
        function validatePhone(value, showError = true) {
            ...
            if (showError) {
                if (!isValid) {
                    ...
                    if (digits.length === 0) errorPhone.textContent = "Введіть номер телефону";
    ...
    modal.querySelector('.calc-modal__window').addEventListener('click', (e) => e.stopPropagation());
